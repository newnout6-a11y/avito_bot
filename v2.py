            ...
            if html_text:
                for ad in self._parse_top(html_text, limit=12):
                    ...
                    for sub in list(self.subscribers.values()):
                        ...
                        try:
                            if ad.image_url:
                                await self.bot.send_photo(sub.user_id, photo=ad.image_url, caption=caption, disable_notification=False)
                            else:
                                await self.bot.send_message(sub.user_id, caption, disable_web_page_preview=True)
                            if self.on_deliver:
