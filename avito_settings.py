...
# «Только новые»: отбрасывать объявления с item-ID сильно ниже фронтира свежести.
...
# item-ID коррелирует с датой создания, но НЕ строго: замер 04.09.2026 показал
# разброс до 68 млн между ID объявлений-ровесников (ID раздаются из нескольких
# диапазонов). Поэтому калитка грубая — режет только заведомо древнее, а не
# «всё, что ниже максимума»: иначе один выброс закрывает ленту полностью.
ONLY_NEW_ID_GATE = os.getenv("ONLY_NEW_ID_GATE", "1") == "1"
# Насколько ниже фронтира ID должен быть, чтобы считаться старьём.
# ~6.2 млн ID/сутки (замер), т.е. 100 млн ≈ две с лишним недели. Должен
# заведомо превышать естественный разброс диапазонов (~68 млн), иначе будут
# ложные отсевы свежих объявлений.
ONLY_NEW_ID_MARGIN = int(os.getenv("ONLY_NEW_ID_MARGIN", "100000000"))
